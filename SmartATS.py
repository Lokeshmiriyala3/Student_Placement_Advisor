...
import joblib
model = joblib.load("ModelRF1.pkl")
...
if st.session_state.result_displayed:
    if st.session_state.success == "Not Placed":
        set_background(failure_image_path)
        st.markdown("<h1 style='text-align: center;'> Placement Prediction: Not Placed ❌</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; font-size: 20px;'>Don't be discouraged. Keep working hard!</p>", unsafe_allow_html=True)
    elif st.session_state.success == "3-5 LPA":
        set_background(success_image_path)
        st.markdown("<h1 style='text-align: center;'> Placement Success 🎉</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; font-size: 20px;'>Congratulations! You are likely to get a placement with a package of 3-5 LPA.</p>", unsafe_allow_html=True)
    elif st.session_state.success == "5-10 LPA":
        set_background(success_image_path)
        st.markdown("<h1 style='text-align: center;'> Placement Success 🎉</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; font-size: 20px;'>Congratulations! You are likely to get a placement with a package of 5-10 LPA.</p>", unsafe_allow_html=True)
    elif st.session_state.success == "10+ LPA":
        set_background(success_image_path)
        st.markdown("<h1 style='text-align: center;'> Placement Success 🎉</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; font-size: 20px;'>Congratulations! You are likely to get a placement with a package of 10+ LPA.</p>", unsafe_allow_html=True)
    ...
    status = st.selectbox("Status", ["Current Student", "Alumni"])
    ...
    placed=st.selectbox("Your Expectation Towards Placement Salary?", ["Not Placed", "3-5 LPA", "5-10 LPA","10+ LPA"])
     
    if st.button("Predict Placement Status"):

        data = [
            year_of_graduation, percentage_10th, percentage_12th,  seat_allocation,
            cgpa_btech, eamcet_rank, interview_confidence, certifications_completed, nptel_courses, gaming,
            hours_gaming, physical_activities, hours_physical, coding_skills, hours_coding, projects_completed,
            hackathons_participated, hackathons_won, hours_placement_preparation, mock_interviews, communication_skills,
            backlogs, cleared_backlogs_years, merit_scholarship, hours_social_media, stipend_received, tech_skills,
            coding_competitions, project_contribution, real_time_project, family_income, attendance, siblings,
            faculty_doubts, area, medium, father_occupation, classmate_doubts, public_speaking, 
        ]
    
    # Encoding string features using LabelEncoder
        le = LabelEncoder()
        data = [le.fit_transform([x])[0] if isinstance(x, str) else x for x in data]
        scaled_data = scaler.transform([data])  # Scaling the features

    # Making the prediction
        prediction = model.predict(scaled_data)[0]

    # Handling different predictions and updating session state
        if prediction == 0:  # Assuming 0 represents "Not Placed"
            st.session_state.success = "Not Placed"
        elif prediction == 1:  # Assuming 1 represents "3-5 LPA"
            st.session_state.success = "3-5 LPA"
        elif prediction == 2:  # Assuming 2 represents "5-10 LPA"
            st.session_state.success = "5-10 LPA"
        elif prediction == 3:  # Assuming 3 represents "10+ LPA"
            st.session_state.success = "10+ LPA"
    
        st.session_state.result_displayed = True
        st.rerun()  # Refresh the page to display results
